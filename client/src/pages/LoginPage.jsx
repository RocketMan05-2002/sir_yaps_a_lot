...
const LoginPage = () => {
  ...
  return (
    ...
    sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      ...
      {/* right */}
      <form onSubmit={onSubmitHandler} className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 
      rounded-lg shadow-lg'>
        ...
        {/* once the name,email,password are entered the below field would be hidden */}
        {
        ...
        {
          currentState === "Sign up" && isDataSubmitted && (
            <textarea rows={4} className='p-2 border-gray-500 rounded-md focus:outline-none
            focus:ring-2 focus:ring-indigo-500'placeholder='Tell a short bio'
        ...
        >{currentState === "Sign up" ? "Create Account" : "Login Now"}</button>

        <div className='flex items-center gap-2 text-sm text-gray-300'>
          <input type='checkbox' />
          <p className='w-[200px] flex items-start text-justify'>Agree to these terms of policy, we both know you aren't reading this, so let me rather write nothing but iuc iuasg basikcvdsjhmbvjhsvcbikbdlb.</p>
        </div>
        ...
        <div className='flex flex-col gap-2'>
          {currentState==="Sign up" ? (
            ...
            onClick={()=>{setCurrentState("Log in"); setIsDataSubmitted(false)}}>Login Here</span></p>
          ) : (
